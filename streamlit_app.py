...
try:
    ...
    if {"actual_price", "predicted_price"}.issubset(set(mlp_pred_df.columns)):
        st.subheader("7) Our Model Accuracy For Games Above $4.20")
        threshold = 4.2
        above_threshold_df = mlp_pred_df[mlp_pred_df["actual_price"] > threshold].copy()

        if len(above_threshold_df) == 0:
            st.info("No games in the prediction CSV have actual price above $4.20.")
        else:
            ...
            acc_col_1.metric("Games with actual price > $4.20", f"{len(above_threshold_df):,}")
            acc_col_2.metric("Correctly predicted as > $4.20", f"{correct_above:,}")
            acc_col_3.metric("Accuracy for this group", f"{accuracy_above:.2%}")

            pie_df = pd.DataFrame(
                {
                    "Outcome": [
                        "Correctly predicted above $4.20",
                        "Missed above-$4.20 games",
                    ],
            ...
            st.caption(
                "Definition used: among games with actual price > $4.20, a prediction is counted as correct when predicted price is also > $4.20."
            )
